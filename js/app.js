(() => {
    ...
    da.init();
    let advantagesItems = document.querySelectorAll(".advantages-hero__item span");
    let delay = 0;
    advantagesItems.forEach((item => {
        delay += .5;
        item.style.animationDelay = `${delay}s`;
    }));
    let stepCards = document.querySelectorAll(".item-how");
    stepCards.forEach((function(stepCard, index) {
        let itemNumber = stepCard.querySelector(".item-how__number span");
        if (itemNumber) itemNumber.innerText = (index + 1).toString();
    }));
    const blogTagsContainers = document.querySelectorAll(".card-blog__tags");
    for (const container of blogTagsContainers) {
        const tags = container.querySelectorAll(".card-blog__tag");
        const containerWidth = container.getBoundingClientRect().width;
        if (containerWidth <= 365) for (let i = 2; i < tags.length; i++) tags[i].parentNode.removeChild(tags[i]);
    }
    window["FLS"] = true;
    isWebp();
    addLoadedClass();
